...
# ---------------------------------------------------------------------------
# Synthetic FHIR helpers used by TestReconciliationContract
# ---------------------------------------------------------------------------

def _make_fhir_condition(
    condition_id: str,
    patient_id: str,
    snomed_code: str,
    snomed_display: str,
    icd10_code: str,
    derived_from_ids: list | None = None,
    clinical_status_code: str = "55561003",
    clinical_status_display: str = "Active",
    clinical_status_text: str = "Active",
) -> dict:
    """Return a minimal but schema-valid FHIR Condition dict."""
    payload = {
        "resourceType": "Condition",
        "id": condition_id,
        "clinicalStatus": {
        ...
        "onsetPeriod": {"start": "2020-01-15T10:00:00.000000"},
    }
    if derived_from_ids:
        payload["extension"] = [
            {
...
class TestReconciliationContract:
    """TDD contracts for dynamic reconciliation logic (Phase 1 — all fail until implemented)."""

    def test_reconciliation_dedupes_conditions_with_shared_primary_or_alternative_code(
        self,
    ):
        """
        Two ingested conditions that share the same primary SNOMED code must collapse
        to a single canonical row after reconcile_conditions().
        Fails with AttributeError until reconcile_conditions is implemented.
        """
        patient_id = "reconcile-dedup-001"
        store = ConditionStore()
        try:
            # Condition A: diabetes mellitus — first record
            cond_a = _make_fhir_condition(
                condition_id="cond-dedup-a",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            )
            # Condition B: same SNOMED/ICD-10 codes, different FHIR id
            cond_b = _make_fhir_condition(
                condition_id="cond-dedup-b",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            ...
            store.ingest_raw_batch(patient_id, [cond_a, cond_b])

            # Pre-reconciliation: staging should hold 2 rows
            pre_reconcile = store.query_current_conditions(patient_id)
            assert len(pre_reconcile) == 2, (
                f"Expected 2 staging rows before reconciliation, got {len(pre_reconcile)}"
            )

            # This call does not exist yet — expected to raise AttributeError
            store.reconcile_conditions(patient_id)

            # Post-reconciliation: one canonical row per code group
            post_reconcile = store.query_current_conditions(patient_id)
            assert len(post_reconcile) == 1, (
                f"Reconciliation must collapse duplicate codes to 1 row, got {len(post_reconcile)}"
            )
        finally:
            store.close()

    def test_reconciliation_uses_derived_from_lineage_to_select_current_truth(self):
        """
        When condition B declares derived-from lineage pointing at condition A,
        reconcile_conditions must prefer B (the child/descendant) as canonical and
        exclude A from query_current_conditions.
        Fails with AttributeError until reconcile_conditions is implemented.
        """
        patient_id = "reconcile-lineage-001"
        store = ConditionStore()
        try:
            # Condition A: the original/parent record
            cond_a = _make_fhir_condition(
                condition_id="cond-lineage-parent",
                patient_id=patient_id,
                snomed_code="54150009",
                snomed_display="Upper respiratory infection",
                icd10_code="J06.9",
            )
            # Condition B: child/descendant — derived from A, same codes
            cond_b = _make_fhir_condition(
                condition_id="cond-lineage-child",
                patient_id=patient_id,
                snomed_code="54150009",
                snomed_display="Upper respiratory infection",
                icd10_code="J06.9",
                derived_from_ids=["cond-lineage-parent"],
            )
            ...
            results = store.query_current_conditions(patient_id)
            result_ids = {r["condition_id"] for r in results}

            # Child (derived) should survive; parent should be superseded
            assert "cond-lineage-child" in result_ids, (
                "Derived child condition must be kept as canonical record"
            )
            assert "cond-lineage-parent" not in result_ids, (
                "Parent condition superseded by derived child must be excluded"
            )
        finally:
            store.close()

    def test_reconciliation_preserves_existing_correction_mask_semantics(self):
        """
        A correction mask applied before reconcile_conditions must still exclude
        the masked condition after reconciliation runs.
        Fails with AttributeError until reconcile_conditions is implemented.
        """
        patient_id = "reconcile-correction-001"
        store = ConditionStore()
        try:
            cond_a = _make_fhir_condition(
                condition_id="cond-corr-a",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            ...
            # Mask cond_a before reconciliation
            store.issue_correction(
                patient_id=patient_id,
                condition_ids=["cond-corr-a"],
                correction_type="EXCLUDE",
                reason="User correction before reconcile",
            )

            # Verify correction is applied in pre-reconcile view
            pre = store.query_current_conditions(patient_id)
            pre_ids = {r["condition_id"] for r in pre}
            assert "cond-corr-a" not in pre_ids, "Correction must mask cond-corr-a pre-reconcile"

            # This call does not exist yet — expected to raise AttributeError
            store.reconcile_conditions(patient_id)

            # Post-reconcile: masked condition must remain excluded
            post = store.query_current_conditions(patient_id)
            post_ids = {r["condition_id"] for r in post}
            assert "cond-corr-a" not in post_ids, (
                "Reconciliation must not resurrect user-corrected (masked) conditions"
            )
        finally:
            store.close()

    def test_query_conditions_contract_stable_after_reconciliation_enabled(self):
        """
        After reconcile_conditions(), query_current_conditions() must still return
        dicts containing the required keys: condition_id, patient_id, code_text,
        clinical_status, primary_code.
        Fails with AttributeError until reconcile_conditions is implemented.
        """
        patient_id = "reconcile-contract-001"
        store = ConditionStore()
        try:
            cond = _make_fhir_condition(
                condition_id="cond-contract-a",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            )
            store.ingest_raw_batch(patient_id, [cond])

            # This call does not exist yet — expected to raise AttributeError
            store.reconcile_conditions(patient_id)

            results = store.query_current_conditions(patient_id)
            assert len(results) >= 1, "At least one condition must remain after reconciliation"

            required_keys = {"condition_id", "patient_id", "code_text", "clinical_status", "primary_code"}
            for row in results:
                missing = required_keys - set(row.keys())
                assert not missing, (
                    f"query_current_conditions dict missing required keys after reconcile: {missing}"
                )
        finally:
            store.close()
    def test_reconciliation_artifacts_store_group_membership_and_decision_basis(self):
        """
        After reconcile_conditions(), the DuckDB tables condition_reconciliation_groups
        and condition_reconciliation_members must exist and contain at least one group
        row with the required columns.
        """
        patient_id = "reconcile-artifacts-001"
        store = ConditionStore()
        try:
            # Two conditions sharing the same SNOMED code → one overlap group
            cond_a = _make_fhir_condition(
                condition_id="cond-artifact-a",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            ...
            # Directly inspect the reconciliation groups table
            with store._db_lock:
                group_rows = store.conn.execute(
                    "SELECT group_id, patient_id, canonical_condition_id, is_ambiguous, decision_basis "
                    "FROM condition_reconciliation_groups WHERE patient_id = ?",
                    [patient_id],
                ).fetchall()
                member_rows = store.conn.execute(
                    "SELECT group_id, condition_id "
                    "FROM condition_reconciliation_members WHERE patient_id = ?",
                    [patient_id],
                ).fetchall()

            assert len(group_rows) >= 1, (
                "condition_reconciliation_groups must have at least one row after reconcile_conditions"
            )
            group = group_rows[0]
            group_id, grp_patient_id, canonical_id, is_ambiguous, decision_basis = group
            assert group_id is not None, "group_id must be set"
            assert grp_patient_id == patient_id, "patient_id must match"
            assert canonical_id is not None, "canonical_condition_id must be set"
            assert isinstance(is_ambiguous, bool), "is_ambiguous must be a bool"
            assert decision_basis is not None, "decision_basis must be set"

            assert len(member_rows) >= 1, (
                "condition_reconciliation_members must have at least one row after reconcile_conditions"
            )
            # Verify member rows have the required columns
            for member_group_id, cond_id in member_rows:
                assert member_group_id is not None, "group_id on member must be set"
                assert cond_id is not None, "condition_id on member must be set"
        finally:
            store.close()

    def test_reconciliation_artifacts_mark_ambiguous_groups(self):
        """
        When two conditions share a code but have no lineage linking them,
        the reconciliation group must be marked is_ambiguous=True with
        decision_basis='code_overlap_no_lineage'.
        A third standalone condition (different code) must be in a separate group
        marked is_ambiguous=False.
        """
        patient_id = "reconcile-ambiguous-001"
        store = ConditionStore()
        try:
            # Overlap pair: same SNOMED, no lineage → ambiguous
            cond_a = _make_fhir_condition(
                condition_id="cond-amb-a",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            ...
            cond_b = _make_fhir_condition(
                condition_id="cond-amb-b",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            )
            # Standalone: different code → no overlap
            cond_c = _make_fhir_condition(
                condition_id="cond-amb-c",
                patient_id=patient_id,
                snomed_code="54150009",
                snomed_display="Upper respiratory infection",
                icd10_code="J06.9",
            )
            store.ingest_raw_batch(patient_id, [cond_a, cond_b, cond_c])
            store.reconcile_conditions(patient_id)

            with store._db_lock:
                group_rows = store.conn.execute(
                    "SELECT group_id, canonical_condition_id, is_ambiguous, decision_basis, member_count "
                    "FROM condition_reconciliation_groups WHERE patient_id = ? ORDER BY member_count DESC",
                    [patient_id],
                ).fetchall()

            assert len(group_rows) >= 2, (
                f"Expected at least 2 groups (one overlap + one standalone), got {len(group_rows)}"
            )

            # The group with 2 members (the overlap pair) must be ambiguous
            overlap_group = next(
                (g for g in group_rows if g[4] >= 2), None
            )
            assert overlap_group is not None, "Must have a group with 2+ members for the overlap pair"
            _, canonical_id, is_ambiguous, decision_basis, member_count = overlap_group
            assert is_ambiguous is True, (
                f"Overlap group with no lineage must be is_ambiguous=True, got {is_ambiguous}"
            )
            assert decision_basis == "code_overlap_no_lineage", (
                f"Expected decision_basis='code_overlap_no_lineage', got {decision_basis!r}"
            )

            # The standalone condition must be in its own group marked not ambiguous
            standalone_group = next(
                (g for g in group_rows if g[4] == 1), None
            )
            assert standalone_group is not None, "Must have a singleton group for the standalone condition"
            assert standalone_group[2] is False, "Singleton group must not be ambiguous"
        finally:
            store.close()

    def test_reconciliation_prefers_lineage_descendant_when_parent_child_conflict(self):
        """
        When condB is derived-from condA (child of A) and they share the same code,
        reconcile_conditions must select condB as canonical and exclude condA.
        """
        patient_id = "reconcile-lineage-depth-001"
        store = ConditionStore()
        try:
            cond_a = _make_fhir_condition(
                condition_id="cond-depth-parent",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            ...
            cond_b = _make_fhir_condition(
                condition_id="cond-depth-child",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
                derived_from_ids=["cond-depth-parent"],
            )
            store.ingest_raw_batch(patient_id, [cond_a, cond_b])
            store.reconcile_conditions(patient_id)

            # Check reconciliation groups table directly
            with store._db_lock:
                group_row = store.conn.execute(
                    "SELECT canonical_condition_id FROM condition_reconciliation_groups "
                    "WHERE patient_id = ? AND member_count >= 2",
                    [patient_id],
                ).fetchone()
            assert group_row is not None, "No multi-member group found"
            assert group_row[0] == "cond-depth-child", (
                f"Expected cond-depth-child as canonical, got {group_row[0]}"
            )

            results = store.query_current_conditions(patient_id)
            result_ids = {r["condition_id"] for r in results}
            assert "cond-depth-child" in result_ids, "Child condition must be in results"
            assert "cond-depth-parent" not in result_ids, "Parent must be excluded by lineage rule"

            stats = store.get_statistics(patient_id)
            assert stats["reconciliation_lineage_overrides"] >= 1, (
                f"Expected at least 1 lineage override, got {stats['reconciliation_lineage_overrides']}"
            )
        finally:
            store.close()

    def test_reconciliation_falls_back_to_stable_rank_when_no_lineage(self):
        """
        Two conditions sharing the same code with no lineage and the same status
        must resolve to a single canonical via stable alphabetical tiebreak.
        """
        patient_id = "reconcile-stable-rank-001"
        store = ConditionStore()
        try:
            cond_a = _make_fhir_condition(
                condition_id="cond-stable-aaa",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            ...
            assert len(results) == 1, (
                f"Expected exactly 1 canonical result, got {len(results)}"
            )
            # Alphabetically first condition_id must be selected
            assert results[0]["condition_id"] == "cond-stable-aaa", (
                f"Expected cond-stable-aaa (alphabetically first), got {results[0]['condition_id']}"
            )
        finally:
            store.close()

    def test_reconciliation_flags_ambiguous_when_multiple_candidates_tie(self):
        """
        Two Active conditions sharing a code with no lineage must produce
        is_ambiguous=True in condition_reconciliation_groups.
        """
        patient_id = "reconcile-ambiguous-flag-001"
        store = ConditionStore()
        try:
            cond_a = _make_fhir_condition(
                condition_id="cond-tie-aaa",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            ...
            with store._db_lock:
                group_row = store.conn.execute(
                    "SELECT is_ambiguous FROM condition_reconciliation_groups "
                    "WHERE patient_id = ? AND member_count >= 2",
                    [patient_id],
                ).fetchone()
            assert group_row is not None, "No multi-member group found"
            assert group_row[0] is True, (
                f"Expected is_ambiguous=True for tied group, got {group_row[0]}"
            )

            stats = store.get_statistics(patient_id)
            assert stats["reconciliation_ambiguous_groups"] >= 1, (
                f"Expected >= 1 ambiguous group in stats, got {stats['reconciliation_ambiguous_groups']}"
            )
        finally:
            store.close()

    def test_query_current_conditions_returns_one_row_per_reconciliation_group(self):
        """
        Three conditions — two sharing code X, one unique code Y — must produce
        exactly 2 rows from query_current_conditions after reconcile_conditions.
        """
        patient_id = "reconcile-one-per-group-001"
        store = ConditionStore()
        try:
            cond_x1 = _make_fhir_condition(
                condition_id="cond-x-first",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            )
            cond_x2 = _make_fhir_condition(
                condition_id="cond-x-second",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            )
            cond_y = _make_fhir_condition(
                condition_id="cond-y-unique",
                patient_id=patient_id,
                snomed_code="54150009",
                snomed_display="Upper respiratory infection",
                icd10_code="J06.9",
            )
            store.ingest_raw_batch(patient_id, [cond_x1, cond_x2, cond_y])

            # Before reconciliation: 3 conditions visible
            pre = store.query_current_conditions(patient_id)
            assert len(pre) == 3, f"Expected 3 pre-reconcile rows, got {len(pre)}"

            store.reconcile_conditions(patient_id)

            post = store.query_current_conditions(patient_id)
            assert len(post) == 2, (
                f"Expected exactly 2 rows (one per reconciliation group), got {len(post)}"
            )
        finally:
            store.close()

    def test_issue_correction_masks_targeted_condition_before_group_selection(self):
        """
        Masking the alphabetically-first condition (would-be canonical) then running
        reconcile must result in the OTHER condition being the canonical — not zero rows.
        """
        patient_id = "reconcile-correction-preselect-001"
        store = ConditionStore()
        try:
            # cond-mask-aaa would be alphabetically first (and thus canonical without correction)
            cond_a = _make_fhir_condition(
                condition_id="cond-mask-aaa",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            ...
            # Mask the alphabetically-first condition
            store.issue_correction(
                patient_id=patient_id,
                condition_ids=["cond-mask-aaa"],
                reason="User correction before reconcile",
            )
            ...
            pre = store.query_current_conditions(patient_id)
            pre_ids = {r["condition_id"] for r in pre}
            assert "cond-mask-aaa" not in pre_ids, "Mask must exclude cond-mask-aaa pre-reconcile"
            assert "cond-mask-zzz" in pre_ids, "cond-mask-zzz must remain visible pre-reconcile"
            ...
            # Must not be zero rows — the surviving condition should be canonical
            assert len(post) == 1, (
                f"Expected exactly 1 row after correction+reconcile, got {len(post)}"
            )
            assert "cond-mask-zzz" in post_ids, (
                "The non-masked condition must survive as the canonical row"
            )
            assert "cond-mask-aaa" not in post_ids, (
                "The corrected condition must remain excluded after reconcile"
            )
        finally:
            store.close()

    def test_public_async_ingestion_api_unchanged_with_reconciliation_enabled(self):
        """
        Background worker + enqueue + flush + reconcile must all work together
        and return conditions with required keys.
        """
        patient_id = "reconcile-async-compat-001"
        store = ConditionStore()
        try:
            store.start_background_worker()
            cond = _make_fhir_condition(
                condition_id="cond-async-compat-a",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            )
            store.enqueue_raw_batch(patient_id, [cond])
            flushed = store.flush_background_ingestion(timeout=10.0)
            assert flushed, "flush must complete within timeout"

            store.reconcile_conditions(patient_id)

            results = store.query_current_conditions(patient_id)
            assert len(results) > 0, "Must return at least one condition"
            ...
            required_status_keys = {
                "worker_running", "queue_depth", "jobs_enqueued",
                "jobs_processed", "jobs_failed", "last_error",
                "last_processed_at", "last_heartbeat",
            }
            missing_status_keys = required_status_keys - set(bg_status.keys())
            assert not missing_status_keys, (
                f"get_background_ingestion_status() missing keys: {missing_status_keys}"
            )
        finally:
            store.stop_background_worker()
            store.close()
...
class TestReconciliationRollback:
    """Kill-switch / rollback safety controls for reconciliation (Phase 6)."""

    def test_reconciliation_mode_off_falls_back_to_unreconciled_live_view(self):
        """
        After mode="on" reconciliation, calling mode="off" must delete all
        reconciliation groups so query_current_conditions returns ALL conditions,
        not just canonicals.
        """
        patient_id = "rollback-mode-off-001"
        store = ConditionStore()
        try:
            with open("conditions.json") as fh:
                all_conds = json.load(fh)
            # Use first 5 real conditions
            first_five = all_conds[:5] if isinstance(all_conds, list) else list(all_conds.values())[:5]
            store.ingest_raw_batch(patient_id, first_five)

            # Establish baseline — number of unreconciled conditions
            pre_count = len(store.query_current_conditions(patient_id))
            assert pre_count > 0, "Should have ingested some conditions"

            # Reconcile with mode="on"
            store.reconcile_conditions(patient_id, mode="on")
            # Mode is now "on"
            assert store.get_reconciliation_mode(patient_id) == "on"

            # Now turn reconciliation off
            store.reconcile_conditions(patient_id, mode="off")

            # Mode should be reported as "off"
            assert store.get_reconciliation_mode(patient_id) == "off", (
                "After mode='off', get_reconciliation_mode must return 'off'"
            )

            # Results must fall back to unreconciled (all conditions)
            off_results = store.query_current_conditions(patient_id)
            assert len(off_results) == pre_count, (
                f"After mode='off', must return all {pre_count} conditions "
                f"(unreconciled), got {len(off_results)}"
            )
        finally:
            store.close()

    def test_reconciliation_mode_shadow_computes_but_does_not_filter_results(self):
        """
        mode="shadow" must compute and store reconciliation groups (so statistics
        show them) but must NOT mark any member as canonical, so
        query_current_conditions returns all conditions unfiltered.
        """
        patient_id = "rollback-mode-shadow-001"
        store = ConditionStore()
        try:
            # Two conditions sharing the same SNOMED code → they form 1 merged group
            cond_a = _make_fhir_condition(
                condition_id="shadow-cond-a",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            ...
            result = store.reconcile_conditions(patient_id, mode="shadow")
            assert result["status"] == "shadow", (
                f"reconcile_conditions(mode='shadow') must return status='shadow', got {result['status']!r}"
            )

            # Mode should be "shadow"
            assert store.get_reconciliation_mode(patient_id) == "shadow", (
                "After mode='shadow', get_reconciliation_mode must return 'shadow'"
            )

            # Groups were computed (groups_computed >= 1)
            stats = store.get_statistics(patient_id)
            assert stats["reconciliation_groups_total"] >= 1, (
                "Shadow mode must still compute and store reconciliation groups"
            )

            # But query must serve BOTH conditions (no canonical filtering)
            shadow_results = store.query_current_conditions(patient_id)
            assert len(shadow_results) == 2, (
                f"Shadow mode must return all 2 unfiltered conditions, got {len(shadow_results)}"
            )
        finally:
            store.close()

    def test_reconciliation_error_auto_degrades_to_safe_unreconciled_mode(self):
        """
        If reconciliation raises an exception internally (e.g. broken _rebuild_code_index),
        the error must be caught and logged, DB state cleaned up, and
        query_current_conditions must still work correctly.
        """
        from unittest.mock import patch

        patient_id = "rollback-error-001"
        store = ConditionStore()
        try:
            cond = _make_fhir_condition(
                condition_id="err-cond-a",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            ...
            # Patch _rebuild_code_index to raise
            with patch.object(store, "_rebuild_code_index", side_effect=RuntimeError("simulated failure")):
                result = store.reconcile_conditions(patient_id)

            # Must not raise; must return error status
            assert result["status"] == "error", (
                f"reconcile_conditions must catch the exception and return status='error', got {result!r}"
            )

            # Mode must be "off" (groups cleaned up)
            assert store.get_reconciliation_mode(patient_id) == "off", (
                "After reconciliation error, get_reconciliation_mode must return 'off' (groups cleaned)"
            )

            # Read path must remain functional
            post_results = store.query_current_conditions(patient_id)
            assert len(post_results) == pre_count, (
                f"query_current_conditions must still work after reconciliation error, "
                f"expected {pre_count} conditions, got {len(post_results)}"
            )
        finally:
            store.close()

    def test_correction_after_reconcile_invalidates_groups_and_returns_remaining_conditions(self):
        """Regression: issuing a correction AFTER reconcile must invalidate stale groups.

        After the correction, query_current_conditions must fall back to unreconciled mode
        (returning all remaining uncorrected conditions) rather than returning nothing
        because the persisted canonical was masked.
        """
        patient_id = "rollback-stale-canonical-001"
        store = ConditionStore()
        try:
            # Two conditions sharing the same ICD-10 code — they'll group together.
            # condA alphabetically-first → will be selected as canonical under stable tiebreak.
            condA = _make_fhir_condition(
                condition_id="aa-stale-canonical-a",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            )
            condB = _make_fhir_condition(
                condition_id="bb-stale-canonical-b",
                patient_id=patient_id,
                snomed_code="73211009",
                snomed_display="Diabetes mellitus",
                icd10_code="E11.9",
            )
            store.ingest_raw_batch(patient_id, [condA, condB])
            store.reconcile_conditions(patient_id)

            # Verify reconcile selected condA as canonical (alphabetically first)
            reconciled = store.query_current_conditions(patient_id)
            assert len(reconciled) == 1, f"Expected 1 canonical after reconcile, got {len(reconciled)}"
            assert reconciled[0]["condition_id"] == "aa-stale-canonical-a"

            # Now mask the canonical (condA) — this must invalidate stale group state
            store.issue_correction(patient_id, condition_ids=["aa-stale-canonical-a"],
                                   reason="rollback regression test")

            # Mode must revert to 'off' (groups cleared)
            assert store.get_reconciliation_mode(patient_id) == "off", (
                "Correction must invalidate reconciliation groups (mode must be 'off')"
            )

            # query_current_conditions must return condB (unreconciled fallback),
            # NOT an empty list from a stale canonical pointer.
            remaining = store.query_current_conditions(patient_id)
            assert len(remaining) == 1, (
                f"Expected condB in unreconciled fallback, got {len(remaining)} conditions: {remaining}"
            )
            assert remaining[0]["condition_id"] == "bb-stale-canonical-b", (
                f"Expected condB, got {remaining[0]['condition_id']}"
            )
        finally:
            store.close()